...
8. **`engineering-roadmap-build-order.md`** — The file that governs how the team actually works: 15 phases (0–14), each with milestones, tasks, dependencies, acceptance criteria, and testing requirements. This is read continuously during implementation, not just once. (Corrected 2026-09-08, v4.11 update: this line had read "12 phases (0–11)" since at least the v4.7 Phase 8 insertion — a stale count the v4.6/v4.7 renumberings should have updated and didn't; now corrected to the roadmap's actual current phase count, 0 through 14 inclusive.)
...
| `volume-2-system-architecture.md` | v4.16.2 |
| `volume-3-database-architecture.md` | v4.12.1 |
| `volume-4-ai-intelligence.md` | v5.15 |
...
| `engineering-roadmap-build-order.md` | v4.12 |
| `demo-simulation-environment.md` | v1.1 |

_Last scanned: 2026-08-19, against the repository state on `claude/new-session-fqsad5`. Volume 2 and Volume 3 corrected from a stale v4.0 to their actual current headers (v4.16, v4.12.1) — all other rows re-checked against their own `**Version:**` header lines and already matched. **Roadmap and Volume 4 rows re-scanned and corrected 2026-09-08** (Roadmap v4.0 → v4.11 → v4.12; Volume 4 v4.0 → v5.14.1 → v5.15 — both stale since well before this pass, now corrected to their actual current headers) across two same-day passes: the Phase 12 — Growth & Acquisition Intelligence insertion, and the Phase 8 Context Assembly Proof lock; no other row was independently re-verified this pass, so the remaining rows' accuracy still dates to the 2026-08-19 scan above._
