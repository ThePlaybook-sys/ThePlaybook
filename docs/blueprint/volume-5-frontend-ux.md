...
**Depends on:** Volume 1 (v3.0 — personas, tiers, journeys, chat-first positioning), Volume 2 (v3.0 — API contracts, scoped event system, Redis), Volume 3 (v3.0 — data shapes, `daily_game_intelligence`), Volume 4 (v3.0 — explainability mapping, progressive disclosure, session memory)
**v3.0 note:** `/chat` is now the default landing route (§3), reordering navigation but adding no new architecture — every route already existed. Recommendation Card gained chat-context rendering using Volume 4 §7's four-level progressive disclosure (§5). See `v3.0-amendments-conversational-intelligence.md` §1–§2 for full reasoning.
