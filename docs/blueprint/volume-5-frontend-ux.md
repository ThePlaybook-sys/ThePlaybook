...
**v5.2 note (MINOR):** Phase 6 Milestone 7 (Responsive + Accessibility + Product Polish, plus MANSA product-facing brand alignment) — a polish pass across every existing Phase 6 route/component, not a redesign. Five real, previously-unnoticed visual-system bugs found and fixed: `border-border-default`/`bg-border-default` were never-generated Tailwind classes (only `border.DEFAULT` was configured, generating `border-border`, not a `.default` sub-key) silently dropping every divider border across `AppNav`, `AccountSummary`, `RecommendationCard`, `TrackRecordSummary`, `TimeMachineStage`, `AuthForm`, `OnboardingForm`; `outline-accent-primary`/`hover:border-accent-primary` referenced a token name (`accent-primary`) that doesn't exist (the configured token is `accent`), silently dropping `RecommendationCard`'s hover/focus affordance; most significantly, `StateBadge`'s `bg-state-*/15`/`border-state-*/30` opacity-modifier classes were silently dropped because Tailwind cannot apply an opacity modifier to a bare `var(--x)` hex color — it requires the `rgb(var(--x) / <alpha-value>)` pattern — meaning every Win/Loss/Push/Void/Mixed-Settled/Withdrawn badge in the app had rendered with no background tint or border color at all, text only, since Milestone 1. Fixed by converting `--state-positive`/`--state-negative`/`--state-neutral` to space-separated RGB channels and wrapping them in `rgb(var(...) / <alpha-value>)` in `tailwind.config.ts`. `AppNav` rebuilt as a client component (`usePathname`) with `aria-current="page"` active-state marking and real ≥44px tap targets (was ~25px); `Container` gained an `as` prop so every route now renders exactly one `<main>` landmark alongside `AppNav`'s `<nav>`; a missing page-level `<h1>` on `/history/[displayId]`'s Time Machine branch was added; a global `prefers-reduced-motion` rule was added; every `<summary>` disclosure control gained a real tap target; headline rows next to fixed-width badge columns gained `min-w-0`/`shrink-0` to guarantee wrap-safety instead of overflow for long team names. Product-facing copy renamed "The Playbook" → "MANSA" (metadata titles, onboarding/How-it-Works/Track-Record-empty-state headings, No-Bet/Bankroll-Preservation headlines, the Account "How MANSA Works" link) with the brand's one deliberate visual moment — name, "Sports Intelligence" category line, and the mantra "See the game. Know the market. Own the decision." — placed only on `/sign-in`, per HQ's "do not plaster it" instruction; the repository name, Railway/Supabase identifiers, database schema, and this Blueprint's own historical volume headers/CHANGELOG/PROGRESS entries deliberately remain "The Playbook" (HQ's explicit M7 boundary: product-facing brand alignment only, not an infrastructure migration). No recommendation logic, ranking, semantic state vocabulary, Time Machine reconstruction semantics, or Track Record contract changed. See `CHANGELOG.md`'s v5.2 entry and `PROGRESS.md`'s M7 entry for the full accounting, including debt explicitly preserved (unchanged from M3-M6/Pre-Phase-6).
**v5.1.3 note (PATCH):** Third real-DEV-validation finding on M6 (the v5.1.2 confirmation-redirect defect was HQ's own Supabase Dashboard action, not code — separately resolved): tapping Sign Out on the deployed DEV app redirected to `http://localhost:8080`, `ERR_CONNECTION_REFUSED`. Root cause confirmed, not guessed: `apps/frontend/app/auth/sign-out/route.ts` (and, latently, `/auth/callback/route.ts`, never yet exercised live) built its redirect target from `new URL(path, request.url)`. Under `next start` (this app's production start command) behind Railway's reverse proxy, a Route Handler's `request.url` reflects the internal connection the Node process actually receives — bound to Railway's injected `PORT` (8080, the same value `PROGRESS.md` already recorded Railway injecting on every service) — not the public deployed origin, a documented self-hosted-Next.js/reverse-proxy pitfall (Next.js's own "Proxy Relative URLs" guidance; community-confirmed via `vercel/next.js` discussion #75250). Railway's edge reliably sets `X-Forwarded-Host`/`X-Forwarded-Proto` to the real public values on every request (Railway's own docs). Fixed with a new `apps/frontend/app/lib/request-origin.ts::resolveRequestOrigin()` helper that builds the redirect origin from those forwarded headers (falling back to `request.url`'s own origin only when they're absent, i.e. local `next dev` with no proxy in front), applied to both `/auth/sign-out` and `/auth/callback`. See `CHANGELOG.md`'s v5.1.3 entry and `PROGRESS.md`'s M6 third-defect entry.
**v5.1.2 note (PATCH):** Second real-DEV-validation finding on M6's sign-up flow (the v5.1.1 hang was confirmed fixed): the email confirmation link redirects to `localhost:3000` instead of the deployed DEV site, `ERR_CONNECTION_REFUSED` on a real device. Root cause confirmed via Supabase's own documentation, not guessed: `emailRedirectTo` is only honored if it matches the project's Auth "Redirect URLs" allow-list; an unlisted value silently falls back to the Auth "Site URL" default, which for this project has never been changed off Supabase's own default (`http://localhost:3000`). Confirmed this is not a frontend code defect — `apps/frontend` contains zero hardcoded `localhost` references, and `AuthForm` already derives `emailRedirectTo` from `window.location.origin` dynamically (a new regression test proves this). **This is a Supabase project Auth-dashboard setting this session has no access to change** — no Supabase Management API token exists in this session's environment, and direct HTTPS to any Supabase domain (including the docs/marketing site, confirmed separately from the already-known project-API-subdomain block) is denied by this sandbox's egress policy. Recorded as a blocking action item for HQ (exact two values needed) rather than worked around — see `PROGRESS.md`'s M6 second-defect entry and `CHANGELOG.md`'s v5.1.2 entry.
**v5.1.1 note (PATCH):** Real DEV human validation of M6's sign-in/sign-up flow found "Create Account" hanging forever on "Please wait...". Root cause: `apps/frontend/Dockerfile` never declared `ARG`/`ENV` for `NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY` — Railway's Dockerfile builds are isolated from service variables unless explicitly opted in via `ARG` (confirmed via Railway's own docs), so both were `undefined` when `next build` statically inlined them into the browser bundle, and `createBrowserClient` threw synchronously on first use, before `AuthForm`'s submit handler ever reached `setSubmitting(false)`. Fixed in two places: the Dockerfile now declares both as build ARGs, and `AuthForm.handleSubmit` now wraps the whole call in try/catch/finally so any exception — this one or a future one — always reaches a visible terminal state (an error message, never a silently frozen button) and always clears the pending state. See `CHANGELOG.md` v5.1.1 entry.
