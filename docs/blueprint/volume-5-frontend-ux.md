...
**Server-side data fetching (Milestone 3):** `/today`, `/recommendations`, and the detail route are Server Components that read a session cookie (`pb_session_token`, `apps/frontend/app/lib/api.ts`) via `next/headers`'s `cookies()` and fetch directly from api-gateway over a new `API_GATEWAY_URL` Railway env var (`http://api-gateway.railway.internal:8080` on dev) -- no client-side proxy route and no CORS configuration, since these pages are read-only display, unlike `/demo`'s interactive-tool proxy pattern. `pb_session_token` is a shared primitive M3 needed to exist, not new auth infrastructure: nothing in this milestone issues, refreshes, or validates a session -- it only forwards whatever raw token is present as a Bearer header, exactly the way M6's login UI is expected to populate it. Every fetch helper returns a discriminated `ApiResult<T>` (`ok`/`unauthenticated`/`not_found`/`error`, `apps/frontend/app/lib/api-types.ts`) so a page never conflates an empty result with a still-loading or error state.
...
## 11. API Read Contracts (Phase 6 Milestone 2 -- v5.0.2; grade field added by Milestone 2.1 -- v5.0.3)
