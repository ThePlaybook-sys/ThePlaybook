...
**Milestone 5.6 — Recommendation Lifecycle & Change Communication — PROPOSED, NOT AUTHORIZED (v4.8, 2026-09-04, HQ directive, planning only).** HQ directed a formal definition of what happens when MANSA changes its view after a recommendation has already been activated, ahead of Phase 12 (Beta) — fully specified in the new Volume 4 §9.7 and Volume 3 §5G (`docs/ops/recommendation-lifecycle-spec-2026-09-04.md`). Proposed scope: extend `recommendation_product_lifecycle_events.event_type` (`STRENGTHENED`/`WEAKENED`/`NO_LONGER_QUALIFIES`/`REPLACED`), add `trigger_type`/`trigger_event_data`/`related_recommendation_product_id` to that same table, add a standalone `user_recommendation_placements` table, ratify an explicit "grading is status-blind" policy (already true today by omission — direct code inspection this pass found no status filter anywhere in the grading pipeline), and close the live-dashboard and future-Telegram change-communication gaps Volume 5 does not currently specify. **Placed here as the natural continuation of Phase 5's own product/leg/lifecycle-event layer** (the schema this milestone extends is 5A/5C's own), not folded into Phase 7 (Market Integrity, a different axis — execution/price, not analytical-validity/communication) or Phase 8 (Contextual Performance Intelligence, one of several *inputs* to this milestone's trigger vocabulary, not its owner). **Not authorized to build** — this entry records the proposal and its required placement in the dependency graph (below), matching the exact treatment already given to Market Integrity/Bet Timing before their own eventual scheduling.
