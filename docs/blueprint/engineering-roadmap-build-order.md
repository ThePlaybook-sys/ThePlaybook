...
**v4.12 note (HQ directive, 2026-09-08, planning only):** Phase 8's own section gains a mandatory **Context Assembly Proof** requirement (full spec in Volume 4 §8.6, v5.15) — before Phase 8 can be considered validated, MANSA must demonstrate real per-game context assembly (initial target: Hunter Henry, 2-3 real historical games, every dimension classified JOINED/PARTIAL/UNAVAILABLE) against real data, never fixtures. A new **Phase 8.1 Gate** makes explicit that substantive scoring/modeling may not begin until real per-game performance substrate exists and that proof is meaningfully executable. Two future execution gates (Gate A: BALLDONTLIE reconfirmation once billing is restored; Gate B: MySportsFeeds `game_boxscore` once a real game is played) are defined, not executed. Zero code, zero provider calls, zero schema changes by this entry — documentation only. See `CHANGELOG.md`'s 2026-09-08 entry for full reasoning.
**v4.11 note (HQ directive, 2026-09-08, planning only):** New **Phase 12 — Growth & Acquisition Intelligence** inserted between Phase 11 (Analytics, unchanged) and the previously-numbered Phase 12 (Beta) — full spec below. Beta (formerly Phase 12) and Production Launch (formerly Phase 13) are renumbered to Phases 13-14 respectively — no scope removed, only positions shifted, same discipline as the v4.6/v4.7 renumberings above. **Phase 12 owns**: UTM/campaign/source attribution, first-touch and conversion attribution, signup/activation funnel measurement, referral architecture, lifecycle/re-engagement hooks, experiment/A-B infrastructure, SEO technical foundation, AI-search/GEO discoverability foundation, campaign measurement, cohort/retention analysis, a growth dashboard, and future MANSA-data-driven public content infrastructure. **Boundary, per HQ's explicit instruction**: Phase 11 (Analytics) provides the general analytics/instrumentation foundation; Phase 12 applies that foundation specifically to growth/acquisition/retention — Phase 11's own scope is unchanged by this insertion. **Phase 12 is NOT authorized to build** — this entry records the phase's placement, ownership, and lock principles only, the same "reserve the phase, do not build it" discipline this roadmap already applies to Milestone 5.6 and Phase 8's own Milestones 8.1+. **A baseline subset of Phase 12 is a mandatory Phase 13 (Beta) prerequisite** (per HQ's own Lock Principle 3: Beta should begin with enough attribution/activation instrumentation to avoid losing the earliest real-user learning) — see the updated Phase 13 dependency list; the remainder of Phase 12 may continue building after Beta begins (Lock Principle 4). Five lock principles govern all future Phase 12 work regardless of when it's built: (1) growth systems measure and improve genuine product value; (2) no fake urgency, fake performance claims, misleading betting claims, dark patterns, or fabricated social proof; (3) Beta needs baseline attribution/activation instrumentation before it opens; (4) advanced growth automation can continue post-Beta; (5) the architecture stays compatible with future multi-sport MANSA. This entry is roadmap-only — no Volume 1–5 section is added or amended by it, per HQ's own scope for this directive; a full Blueprint-level spec (mirroring how Phase 8 gained Volume 4 §8.6) remains a future step if HQ wants one. See `CHANGELOG.md`'s 2026-09-08 entry for full reasoning.
...
Phase 8 (Contextual Performance Intelligence) — NOT part of the parallel group above; see note below
   │
Phase 12 (Growth & Acquisition Intelligence) — reserved, NOT authorized to build; see note below
   │
Phase 13 (Beta)
   │
...
**Phase 8 (Contextual Performance Intelligence) is deliberately drawn separately above, not folded into the Phase 7/9/10/11 parallel group** — it is numbered directly after Phase 7 because HQ has authorized its planning now (2026-09-04, v4.7, mirroring Phase 7's own 2026-09-02 precedent), not because it is proven independent of the others the way Twilio/OCR/Analytics are proven independent of Market Integrity. Its real dependency shape is deliberately NOT resolved by this entry: Volume 4 §8.6 documents it as sitting upstream of the already-frozen Probability Modeling Agent (§2.5) and several already-built Context & Data/Matchup agents (§2.2/§2.3), which is a Phase 4 pipeline position, not a standalone Phase-6-onward capability the way Phase 7/9/10/11 are. Whether Milestone 8.1+ implementation actually requires reopening Phase 4 agent wiring is an explicit open question for Phase 8's own future contract-audit milestone, not decided here — **Phase 8 is placed after Phase 7 in sequence and is a mandatory Beta (Phase 13) prerequisite, but this diagram does not claim it can run in parallel with 7/9/10/11 the way those four can run in parallel with each other.**

**Phase 12 (Growth & Acquisition Intelligence, v4.11, 2026-09-08, HQ directive) is placed directly after Phase 11 (Analytics) and before Phase 13 (Beta), reserved but explicitly NOT authorized to build** — it applies Phase 11's general analytics/instrumentation foundation specifically to acquisition/activation/conversion/retention/referral, so it cannot precede Phase 11 in sequence, but its own build is not gated behind Phases 7/9/10 the way Phase 13 (Beta) is gated behind all of 6-11 plus Phase 8. Only a baseline subset (UTM/first-touch attribution, signup/activation funnel measurement) is a mandatory Phase 13 prerequisite, per HQ's own Lock Principle 3 — the remainder of Phase 12's scope (referral architecture, lifecycle/re-engagement hooks, experiment infrastructure, SEO/GEO foundation, cohort/retention analysis, growth dashboard, public content infrastructure) may continue building after Phase 13 begins, per Lock Principle 4.
...
**Context Assembly Proof — mandatory pre-validation artifact (added v4.12, 2026-09-08, HQ directive; full spec in Volume 4 §8.6).** Before this phase can be considered validated, MANSA must demonstrate a real Context Assembly Proof: 2-3 real historical game observations for an initial target player (Hunter Henry), every context dimension (game identity, opponent, home/away, venue, performance, usage/snaps, lineup/depth, injuries/availability, weather, news, odds) classified JOINED/PARTIAL/UNAVAILABLE against real data, never fixture data. This validates the data substrate and joins — it is explicitly NOT the contextual scoring/modeling itself. Volume 4 §8.6's own Context Readiness Matrix (2026-09-08 evidence) currently shows the one dimension that gates everything downstream — real per-game player performance/usage — as BLOCKED; two future execution gates are defined there (Gate A: BALLDONTLIE `/nfl/v1/stats` reconfirmation, blocked on billing; Gate B: MySportsFeeds `game_boxscore`, waiting on a completed real game), neither executed by this entry.

**Phase 8.1 Gate (added v4.12, 2026-09-08, HQ directive):** Milestone 8.1's substantive scoring/modeling work may not begin until real per-game performance substrate exists and the Context Assembly Proof can be executed meaningfully — schema/interface preparation may be separately authorized ahead of that, but fixtures must never be presented as satisfying this gate.

**Acceptance Criteria / Testing Requirements:** To be defined per-milestone as each is authorized, following this project's established fixture-first + live rollback-wrapped SQL proof discipline — not written here in advance of authorization, same principle Phase 7 applied. **The Context Assembly Proof above is the one exception written here in advance**, since HQ's own directive locks it as a required validation artifact for the phase as a whole, not a single milestone's own criteria.
...
**Boundary with Phase 12 (added v4.11, 2026-09-08, HQ directive):** This phase builds the *general* analytics/instrumentation foundation (charting, business-metrics dashboarding, the AI/projected/verified separation). Phase 12 (Growth & Acquisition Intelligence) applies that same foundation specifically to acquisition, activation, conversion, retention, and referral — it is a consumer of what this phase builds, not a duplicate of it. Nothing in this phase's own scope changes because of that insertion.

---

## Phase 12 — Growth & Acquisition Intelligence

**New phase (v4.11, 2026-09-08, HQ directive)** — reserved directly in the roadmap, the same "spec the phase, do not build it yet" discipline already applied to Milestone 5.6 and to Phase 8's own Milestones 8.1+. Inserted between Phase 11 (Analytics, unchanged) and the previously-numbered Phase 12 (Beta, now Phase 13) — see that phase's own renumbering note below. **This entry is roadmap-only; no Volume 1–5 section is added or amended by it** (a real, disclosed departure from the "spec fully in a Volume first, then place on the roadmap" discipline Phase 7 followed and Phase 8 explicitly flagged deviating from — HQ's directive for this phase scoped it as a roadmap update, not a Blueprint amendment).

**Purpose:** Build the instrumentation and systems needed to understand and improve the full growth loop — acquisition → signup → activation → paid conversion → retention → referral.

**Implements:** Not yet mapped to a Volume section (see the roadmap-only note above). Builds on Volume 5 §5 (Charts) and Volume 1 §8 (Success Metrics) infrastructure that Phase 11 establishes.

**This phase owns, at minimum:**
- UTM/campaign/source attribution
- First-touch and conversion attribution
- Signup/activation funnel measurement
- Referral architecture
- Lifecycle/re-engagement hooks
- Experiment/A-B infrastructure
- SEO technical foundation
- AI-search/GEO discoverability foundation
- Campaign measurement
- Cohort/retention analysis
- Growth dashboard
- Future MANSA-data-driven public content infrastructure (the natural growth-side counterpart to Phase 14's own public postgame-review content cadence, Volume 1 §9)

**Boundary:** Phase 11 (Analytics) provides the general analytics/instrumentation foundation. Phase 12 applies that foundation specifically to growth/acquisition/retention — it is placed after Phase 11 in sequence for exactly this reason, not because it is otherwise proven dependent on Phases 7/9/10.

**Lock principles (HQ directive, binding on all future Phase 12 work regardless of when it's built):**
1. Growth systems measure and improve genuine product value.
2. No fake urgency, fake performance claims, misleading betting claims, dark patterns, or fabricated social proof.
3. Beta (Phase 13) should begin with enough attribution/activation instrumentation to avoid losing the earliest real-user learning.
4. Advanced growth automation can continue after Beta if needed.
5. Keep the architecture compatible with future multi-sport MANSA.

**Dependencies:** Phase 11 (Analytics) complete — Phase 12 applies that phase's instrumentation foundation, it doesn't rebuild it. Phase 6 (core frontend) complete, since signup/activation funnel measurement and SEO/GEO foundation both need real product surfaces to instrument.

**Phased authorization, per HQ's explicit "do not implement Phase 12 now" instruction:** **Not authorized to build.** A baseline subset — UTM/campaign/source attribution and signup/activation funnel measurement — is a mandatory Phase 13 (Beta) prerequisite (Lock Principle 3; see Phase 13's updated dependency list) and may be authorized ahead of the rest of this phase, the same phased-authorization pattern already used for Milestone 5.6's schema layer and Phase 8's Milestone 8.0. The remaining scope (referral architecture, lifecycle/re-engagement hooks, experiment/A-B infrastructure, SEO technical foundation, AI-search/GEO discoverability, campaign measurement, cohort/retention analysis, growth dashboard, public content infrastructure) is reserved for after Phase 13 begins (Lock Principle 4), pending its own future authorization.

**Milestones, Key Tasks, Acceptance Criteria, and Testing Requirements:** To be defined per-milestone as each is authorized, following this project's established fixture-first + live rollback-wrapped SQL proof discipline (Phase 7/Phase 8's own precedent) — not written here in advance of authorization.

---

## Phase 13 — Beta

**Renumbered from Phase 11 to Phase 12 at v4.7 (2026-09-04, HQ decision), then from Phase 12 to Phase 13 at v4.11 (2026-09-08, HQ directive, new Phase 12 — Growth & Acquisition Intelligence inserted directly before it)** — scope unchanged, only position shifted each time; its dependency list below now also names Phase 8 and a Phase 12 baseline subset as explicit mandatory prerequisites, not merely part of a numeric range. (Originally Phase 10, renumbered to Phase 11 at v4.6, 2026-09-02, when its dependency list gained Phase 7.)
...
- Baseline Phase 12 attribution/activation instrumentation (UTM/first-touch attribution, signup/activation funnel measurement) confirmed live and capturing real data before cohort recruitment begins (added v4.11, 2026-09-08)
...
**Renumbered from Phase 11 to Phase 12 at v4.6 (2026-09-02), then Phase 12 to Phase 13 at v4.7 (2026-09-04), then Phase 13 to Phase 14 at v4.11 (2026-09-08, HQ directive, new Phase 12 — Growth & Acquisition Intelligence inserted earlier in the sequence)** — scope unchanged each time, only position shifted; its dependency below now reads "Phase 13" (Beta).
...
- Begin the public postgame review content cadence from Volume 1 §9 as an ongoing operational commitment, not a one-time launch asset — this content cadence is the operational counterpart to Phase 12's own reserved "future MANSA-data-driven public content infrastructure" item (added v4.11, 2026-09-08); Phase 12 builds the growth/discoverability infrastructure around content, this milestone is the content itself

**Dependencies:** Phase 13 complete, with legal sign-off and calibration review both closed out.
...
- **Beta access model implementation — required before Phase 13 (added 2026-08-07; phase number corrected v4.7, 2026-09-04, then again v4.11, 2026-09-08 — this entry read "Phase 10" both before and after the v4.6 renumbering, a stale reference to Beta's pre-v4.6 number that the v4.6 pass should have updated to 11 and didn't; corrected to 12 at v4.7, and now corrected again to Beta's current number, 13, following the v4.11 Phase 12 — Growth & Acquisition Intelligence insertion, matching the identical correction made to Production Launch's acceptance criteria above).** Reviewed and decided 2026-08-07: beta access should be modeled as a dedicated `beta_testers` table, not additional columns on `user_profiles`, and integrated with the existing `feature_flags` entitlement system (Volume 3) rather than a separate ad hoc gate — per that review's six-point assessment. No schema changes made yet; this is a placeholder so the decision isn't lost by the time Phase 13 actually starts. Implement as part of Phase 13 setup, ahead of cohort recruitment (Milestone 1) — see the corresponding note in the Phase 13 section above.
- **Phase 12 (Growth & Acquisition Intelligence) — reserved, added 2026-09-08 (v4.11, HQ directive).** New phase inserted between Phase 11 (Analytics) and Phase 13 (Beta) — full spec in the Phase 12 section above. Not authorized to build; a baseline attribution/activation subset is a mandatory Phase 13 prerequisite, the rest is deferred to after Beta begins. Tracked here so the reservation isn't lost before its own future authorization pass.
...
- **Canonical schedule coverage reconciliation — required before any provider-driven ingestion pipeline trusts `games` as complete (added 2026-09-11, HQ directive, surfaced by the Sunday completed-game ingestion design work).** Full incident trail: `docs/ops/phase-8-sunday-canonical-game-coverage-audit-2026-09-11.md` and `docs/ops/phase-8-week1-canonical-schedule-recovery-2026-09-11.md`. MANSA's canonical `games` table currently has no mechanism that reconciles its own contents against an authoritative provider schedule and surfaces gaps — the only reason the 2026 Week 1 slate was found to be 5-of-16 real games (four of the missing eleven a full 1:00 PM ET cluster, the rest scattered across Thu/SNF/MNF) was a manual HQ cross-check against the real official schedule, not anything MANSA itself detected. A real ingestion pipeline (Sunday postgame capture, or any future scheduled-game consumer) that assumes `games` already holds the full real slate for a given week has no structural reason to be right, and no current mechanism would ever tell it otherwise. **Needed**: a reconciliation capability that, given one authoritative schedule response for a week, reports (not silently fixes) missing canonical games, duplicate matchup/time conflicts, provider-id collisions, and canonical rows that have gone stale relative to newer provider data (the concrete example found this pass: the SEA/NE opener's local `status` still reads `'scheduled'` though the real game is long final and its full boxscore is already persisted) — surfacing all four categories as an explicit report a human or a future gated activation step acts on, never assumed-complete by silence. Not yet designed or built; this entry exists so the requirement isn't lost before a future pass scopes it.
