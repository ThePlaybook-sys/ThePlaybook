...
**Status:** APPROVED — IN IMPLEMENTATION (approved by Mac 2026-08-19). DEMO-1 (isolation foundation), DEMO-2 (demo provider-adapter family), DEMO-3 (virtual clock / scenario runner), DEMO-4 (Operator Dashboard + Living Functional Wireframe), and DEMO-5 (Phase 3 scenario library) are BUILT — DEMO-4 additionally live-proven end-to-end in the real `demo` Railway environment against the real `theplaybook-demo` Supabase project (DEMO-4B, 2026-08-20). See Section 19 for per-step status. DEMO-6 onward remain design only; each step requires its own separately-approved execution plan before implementation, per the phase-gating discipline this document was built under. Per Mac's explicit 2026-08-20 direction, primary engineering attention now returns to the real Phase 4 build — Demo Mode does not become a parallel track; DEMO-6 onward proceed only once their gating real phase exists.
...
**Implementation status note (2026-08-20, no version bump — tracking only, not a design change):** DEMO-1, DEMO-2, DEMO-3, DEMO-4, and DEMO-5 built per Section 19's own status markers. DEMO-4B additionally took DEMO-4 live: all three services (`sports-intel-layer`, `api-gateway`, `frontend`) deployed and running in the real `demo` Railway environment, a real reference-data gap found and fixed (demo Supabase's `seasons` table was missing the row `supabase/seed.sql` already defines), a full 7-step scenario run to completion driven by Mac's own browser, every DGI card cross-checked against live demo Supabase and matching exactly, and reset verified live returning every operational table to 0 while the reference taxonomy stayed exactly intact. DEMO-3's own scenario runner/reset logic is now proven against the real `theplaybook-demo` project, not just the in-memory fake. DEMO-5 added a four-scenario library (`pregame_intelligence_evolution`, `team_news_injury_depth_chart`, `provider_outage_resilience`, `postgame_stat_correction`) proving append-only odds/prop/weather history and per-category freshness staleness, injury append-only escalation plus genuine roster-membership team-change detection, per-category provider-outage isolation at the Pregame Worker's coordinated T-minus-5 refresh, and postgame stat-correction (insert-only-on-change team/player stats, `final_score` recomputation) — 29 new tests, full sports-intel-layer regression 611/611. No dedicated postponed/canceled scenario was built; see the Section 19 DEMO-5 entry for why, and for the real Phase 3 product/backend consideration this surfaced instead of solving inside Demo Mode. Not yet built: Component Gallery (blocked on the UX/UI Designer Brief PDF, still not in-repo), Telegram/Twilio demo delivery, Phase 4/5 Demo parity (DEMO-7/DEMO-8, both correctly gated — Phase 4 and Phase 5 are both still NOT STARTED per `PROGRESS.md`'s own Phase Status) — all remain exactly as designed in this document, not implemented.
...
4. **DEMO-4 — Operator Dashboard + Living Functional Wireframe. STATUS: BUILT AND LIVE-PROVEN (2026-08-20).** (Section 10, Section 16) — relabeled ahead of the full scenario library per the 2026-08-19 execution-plan approval's own Decision 5 ("built immediately once DEMO-3 exists rather than waiting"; `PROGRESS.md`'s 2026-08-19 execution-plan entry records this exact resequencing — this document's own numbering below was never updated to match until now). Frontend → API Gateway (`/v1/demo/*`) → internal-authenticated call → sports-intel-layer's Demo router (`/internal/demo/*`) → the existing `ScenarioRunner`/persistence reads, exactly per Section 10's approved data-access model (API-Gateway-only, no direct frontend-to-Supabase reads). Scenario control/read endpoints, a demo-operator token access gate (Mac's live Option A decision — the two existing auth mechanisms in this codebase were inspected and neither fit a demo tool cleanly), the `/demo` Operator Dashboard/games/game-detail pages, structural DEMO MODE banner, raw/debug toggle, and 3s/10s v1 polling (Section 10's approved update mechanism). Component Gallery (`/demo/components`) correctly not built — no UX/UI Designer Brief PDF is in this repository yet, per the standing hard-stop. Proven via 36 new automated tests (sports-intel-layer + API Gateway) plus a clean frontend build/type-check, **then a real live deployment (DEMO-4B, `PROGRESS.md`'s 2026-08-20 entry): all three services running in the `demo` Railway environment, a real reference-data gap found and fixed (demo Supabase's missing `seasons` row), a full 7/7 scenario run to completion driven by Mac's own browser click-through, every DGI card cross-checked against live demo Supabase and matching exactly, and reset verified live returning every operational table to 0 while the reference taxonomy stayed exactly intact.** This is the strongest live-proof standard any DEMO-N step has reached so far — see `PROGRESS.md` for the full evidence trail.
5. **DEMO-5 — Phase 3 scenario library. STATUS: BUILT (2026-08-20).** (Section 7) — four new scenarios beyond DEMO-3's one minimal scenario, each proving a distinct real Phase 3 persistence/coordination behavior never previously exercised end-to-end: `pregame_intelligence_evolution.json` (odds/player-props/weather append-only history across two real polls of the same worker, plus `daily_game_intelligence`'s per-category freshness correctly flipping from fresh to stale after the confirmed 15-minute weather TTL elapses with no further poll); `team_news_injury_depth_chart.json` (`injury_reports`' append-only escalation history, `roster_memberships`' genuine insert-on-team-*change* semantics distinguished from a same-team depth-chart-only reshuffle, `depth_chart_snapshots` written unconditionally every poll); `provider_outage_resilience.json` (the new `fail_categories` mechanism isolating exactly one of the four categories the Pregame Worker's single T-minus-5 coordinated refresh builds, proving the other three still persist and `daily_game_intelligence` still refreshes for them); `postgame_stat_correction.json` (the first scenario to exercise a second reconciliation checkpoint beyond "initial" — the "+10m" checkpoint — proving `team_stats`/`player_stats`' insert-only-on-*change* persistence and `games.final_score`'s recomputation when a correction changes the score). One genuine Demo-mode-only gap found and fixed during this step (not a production defect): `ScenarioRunner._build_weather_adapter` was missing the internal `game_id` → provider-`game_id` resolution hop `DemoWeatherAdapter` needs, since `WeatherConditions` requires an internal id unknown at scenario-authoring time — fixed with a new `_resolve_internal_game_id` helper, zero production code touched. **Real Phase 3 product/backend consideration surfaced, not fixed here per explicit instruction:** postponed/canceled games receive no distinct handling in the Odds/Player Props/Injury/Weather Workers' candidate-game selection (`app.persistence.games.list_games_in_window`, which all four call into, has no `status` filter) — those four workers keep polling a postponed/canceled game exactly like a normal one until it falls out of the kickoff-relative time window; only `find_previous_final_game` (rest-day computation) and the Postgame Worker's own `status == "final"` finalize gate treat status specially. Confirmed by direct code inspection, recorded here for later Phase 3 review, not solved inside Demo Mode. Proven via 29 new tests (`tests/test_demo_scenario_library.py`) covering deterministic replay, worker outcomes, Supabase state, DGI freshness, reset behavior, zero provider HTTP calls, and demo-Supabase-only network access for all four scenarios; full sports-intel-layer regression 611/611. Session lifecycle tooling (start/advance/rewind/end, Section 16) beyond DEMO-4's existing control endpoints was not needed — `run_next_step`/`run_until_checkpoint`/`run_to_completion`/`reset` (DEMO-3/DEMO-4) already covered every new scenario's needs.
...
8. **DEMO-8 — Phase 5 / Time Machine scenario parity** — Scenario 18 (Time Machine reconstruction demo), unlocked only once Phase 5 ships its real Time Machine mechanism. **Corrected (Pre-Phase-6 Operational Readiness Gate, 2026-08-27):** this entry originally named `recommendation_snapshots` as that mechanism — Phase 5's own Milestone 5.3 confirmed that legacy table unfit for the product layer and left it permanently untouched; the real mechanism, shipped in Milestone 5.3, is the activation-snapshot manifest (Volume 3 §5C) plus `app.orchestration.reconstruction.reconstruct_recommendation_product`. DEMO-8 remains unstarted and is not authorized by this correction.
...
DEMO-1 through DEMO-5 were the only steps that could reasonably start early, since they depend only on Phase 3 (already real) — all five are now BUILT. DEMO-6 through DEMO-9 remain hard-gated on later phases per the roadmap's own dependency chain, exactly mirroring CLAUDE.md's phase-gating rule — Demo Mode does not get to skip ahead of the phases it's demonstrating. Per Mac's explicit 2026-08-20 direction, primary engineering attention now returns to the real Phase 4 build; DEMO-7 is updated only once real Phase 4 agent/confidence/consensus/explainability capabilities exist, and DEMO-8 only once real Phase 5 recommendation/parlay/Time Machine capabilities exist — each still requires its own separately-approved execution plan before implementation, per the phase-gating discipline this document was built under.
