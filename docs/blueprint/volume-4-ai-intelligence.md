...
**Depends on:** Volume 2 (v3.0 — Orchestrator deployment shape, async fan-out pattern, scoped event system, Redis) and Volume 3 (v3.0 — `agents`, `agent_performance_scores`, `recommendation_agent_outputs`, `consensus_snapshots`, `model_routing_rules`, `prompt_registry`, `model_registry`, `daily_game_intelligence` tables)
...
**v3.0 note:** Bankroll Coach's stake formula is now explicit fractional Kelly Criterion (§2.5). NL Engine gained session-scoped preference memory and the four-level progressive disclosure spec (§7). Recommendation Strategy Engine's parlay market-mixing freedom made explicit (§9). Agents now query `daily_game_intelligence` first (§3.1). See `v3.0-amendments-conversational-intelligence.md` §2–§5 for full reasoning.
