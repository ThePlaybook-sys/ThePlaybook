...
## 5G. Recommendation Lifecycle & Change Events (v4.28, DECISIONS LOCKED 2026-09-04 — schema itself remains ARCHITECTURE RESERVATION ONLY, nothing described in this section exists in code today)

**HQ directive (2026-09-04): formally define what happens when MANSA changes its view after a recommendation has already been activated.** Full report: `docs/ops/recommendation-lifecycle-spec-2026-09-04.md`. **HQ Decision Lock (2026-09-04, same day):** the design below — vocabulary, `REPLACED` semantics, the status-blind grading policy, `user_recommendation_placements`, and the explicit decision NOT to touch `market_monitoring_events` — is now **approved as the binding design**, not merely a proposal under consideration. See the "HQ Decision Lock" subsection at the end of this section for the exact ratified decisions. **Approving the design is not authorization to build it** — no migration is applied by this entry, no code changes, **PLANNING ONLY**, same discipline as §4.3/§4.4 before their own later implementation entries.
...
```
- **`trigger_type`** reuses `market_monitoring_events.event_type` (§7 below) **verbatim** for its first five values — per HQ's own explicit "do not invent terminology if current schema already implies better names" instruction, this is the closest existing controlled vocabulary and it already anticipates exactly this use. Two values are new, because HQ's directive names two trigger categories `market_monitoring_events` was never designed to cover: `contextual_intelligence_change` (Volume 4 §8.6, Phase 8, not yet built) and `model_refresh` (a routine Strategy/consensus recompute — e.g. an Elite second-pass reconciliation, §4.3 — surfacing a materially different result on the same evidence). Nullable — an `ACTIVATED` event has no trigger; only `STRENGTHENED`/`WEAKENED`/`NO_LONGER_QUALIFIES`/`WITHDRAWN`/`REPLACED` are expected to populate it. **HQ Decision Lock (2026-09-04): `market_monitoring_events` itself is NOT extended, altered, or implemented by this section.** Its own `event_type`/`action_taken` columns stay exactly as §7 already documents them — zero rows, zero code, unchanged. `trigger_type` above is a **separate, own-table CHECK constraint on `recommendation_product_lifecycle_events`** that happens to borrow the same five value-strings for consistency of vocabulary; it is not a foreign key, not a shared enum type, and creating it does not touch `market_monitoring_events`'s schema in any way. Phase 7 (Market Integrity & Anomaly Intelligence) remains the sole owner of `market_monitoring_events` and of actually detecting/writing any of these signals — this section only reserves where such a signal, once real, would be *recorded as having caused a lifecycle event*, not how it gets detected.
...
**Direct code inspection during this pass (`apps/ai-orchestrator/app/persistence/postgame_grading.py`, `apps/ai-orchestrator/app/orchestration/postgame_grading.py`) confirms: no query in the grading pipeline filters by `recommendation_products.status` today.** `read_recommendation_legs_by_game`/`read_recommendation_legs_by_product`/`read_no_bet_products_by_game`/`grade_game`/`_maybe_rollup_product` all read by `game_id`/`recommendation_product_id`/`recommendation_type` only. **A withdrawn product's legs are graded exactly like an active product's, on their frozen activation-time terms.** This was true by omission, not by a documented, tested policy — **HQ Decision Lock (2026-09-04): ratified as a binding, mandatory policy**, not a code change (the code already behaves this way; nothing here alters it):

> **Policy (RATIFIED, 2026-09-04): grading is status-blind by design.** `recommendation_products.status` governs only whether a recommendation is currently presented as actionable — it has no bearing on whether its legs get graded. Once activated, a recommendation remains INDEPENDENTLY gradeable on its ORIGINAL frozen terms even if later `WEAKENED`, `WITHDRAWN`, `NO_LONGER_QUALIFIES`, or `REPLACED`. A replacement/reversal that is itself activated is a SEPARATE, independently gradeable MANSA decision — never merged with, and never a substitute for, the original's own grade. This is mandatory protection against survivorship bias: withdrawal changes what a user is told to do next; it never changes what MANSA is scored on for what it already said at 10:00 AM. A regression test enforcing this (grade a `WITHDRAWN` product's legs, confirm a real grade event is still written) is a required, not optional, deliverable of any future implementation of this section.
...
### Placed-status tracking — approved new table, not an extension of `user_recommendation_selections`

**HQ Decision Lock (2026-09-04): the `user_recommendation_placements` concept below is approved.** It records USER-REPORTED placement/exposure only — never sportsbook-verified execution, since MANSA has no sportsbook integration and none is proposed. Placed status changes COMMUNICATION CONTEXT only: it never alters grading (§ above) or Track Record, and any future implementation must never imply MANSA can cancel, cash out, or hedge the wager on the user's behalf.
...
### HQ Decision Lock (2026-09-04) — full recap

HQ reviewed this section's proposal and locked the following seven decisions the same day (full request/response: `docs/ops/recommendation-lifecycle-spec-2026-09-04.md`). **Locking a decision approves the design; it does not authorize implementation** — every item below remains schema/policy on paper only until a separately-authorized build pass.

1. **Vocabulary approved as proposed:** `STRENGTHENED`/`WEAKENED`/`NO_LONGER_QUALIFIES`/`REPLACED`, added to `recommendation_product_lifecycle_events.event_type`. `recommendation_products.status` stays binary `active`/`withdrawn` — no new status value.
...
3. **Status-blind grading is now mandatory policy, not merely proposed** — see the ratified policy block above. A replacement/reversal is a separate, independently gradeable decision.
4. **`user_recommendation_placements` approved** as a user-reported (not sportsbook-verified) signal; affects communication tone only, never grading; must never imply cancel/cash-out/hedge capability.
5. **`market_monitoring_events` is explicitly NOT touched, extended, or implemented by this section** — Phase 7 remains its sole owner; `trigger_type` above is a separate, own-table CHECK that merely borrows the same value-strings.
6. **Milestone 5.6 approved as mandatory pre-Beta** (Engineering Roadmap v4.9) — with a phased authorization: basic lifecycle mechanics (this section's schema, the event-type/trigger_type vocabulary, `user_recommendation_placements`) may be implemented ahead of Phase 7/8, but Milestone 5.6 cannot be considered CLOSED until real Phase 7 (`market_monitoring_events`-sourced) and Phase 8 (contextual-intelligence) signals can actually feed `trigger_type` — see the roadmap's own updated Milestone 5.6 entry for the exact split.
7. **Dashboard behavior locked at the principle level:** a materially changed recommendation must never silently disappear or overwrite its previous state on any surface (dashboard, Time Machine, future Telegram). Exact visual treatment remains an open future implementation/design decision, not decided here.
...
- Does not wire, extend, or implement `market_monitoring_events`/`worker-market-monitor` (still zero rows, zero code, per Phase 7 Milestone 7.0's own audit — Phase 7 remains sole owner) or Volume 4 §9.5's execution states.
- Does not create any migration — every table/column above is an HQ-approved design pending a separate implementation authorization (see roadmap update, same date).
