...
- [ ] Phase 0 — Repository, Environments, CI/CD — IN PROGRESS
      ...
      - [ ] Sentry (or equivalent) wired to all services — IN PROGRESS: DSN provided by Mac and wired to `api-gateway` only (env var + SDK init in all 3 environments); `ai-orchestrator`, `sports-intel-layer`, `workers`, and `frontend` still need it, see Notes
...
- **2026-08-06 — Sentry wired for `api-gateway`.** Mac provided a Sentry DSN (FastAPI project). Set as `SENTRY_DSN` on the `api-gateway` Railway service across dev/staging/production (same pattern as other API keys — Railway env var, never committed). Added `sentry-sdk[fastapi]` to `apps/api-gateway/requirements.txt` and `sentry_sdk.init(dsn=os.environ.get("SENTRY_DSN"), send_default_pii=True)` before app construction in `apps/api-gateway/app/main.py`, plus a `/sentry-debug` verification route. Still outstanding: `ai-orchestrator`, `sports-intel-layer`, `workers`, and `frontend` (JS SDK) all need the same treatment before Phase 0's error-tracking acceptance criterion is fully met — not done yet since only `api-gateway` was requested this round.
