...
if os.environ.get("RAILWAY_ENVIRONMENT_NAME", "dev") == "dev":
    ...
    if os.environ.get("RUN_MSF_TEAM_SEASON_STATS_ACTIVATION") == "1":
        import json
        import logging

        _msf_team_season_stats_logger = logging.getLogger(
            "sports-intel-layer.diagnostics.msf_team_season_stats_activation"
        )

        @app.on_event("startup")
        async def _run_msf_team_season_stats_activation_once() -> None:
            """TEMPORARY, one-shot activation hook for MANSA Phase 8.3A's
            HQ-authorized controlled DEV team season stats activation
            (2026-09-08, see `app.diagnostics.
            msf_team_season_stats_activation`'s own module docstring).
            Zero live MySportsFeeds calls -- the real, already-captured
            team_stats_totals/standings bodies embedded in that module
            are the only input. Dev-only mount, gated behind
            `RUN_MSF_TEAM_SEASON_STATS_ACTIVATION == "1"`. Guarded by a
            new `activation_run_markers` row claimed before any real
            write, closing the overlapping-Railway-deployment duplicate-
            execution gap every prior Phase 8.2 activation pass disclosed
            but did not fix. Real, durable write via the real
            `persist_team_season_stats()` path -- reverting this file
            afterward removes only the temporary wiring, never the
            resulting rows."""
            from app.diagnostics.msf_team_season_stats_activation import (
                run_msf_team_season_stats_activation,
            )

            result = await run_msf_team_season_stats_activation()

            _msf_team_season_stats_logger.warning("MSF_TEAM_SEASON_STATS_ACTIVATION_START")
            _msf_team_season_stats_logger.warning(
                "MSF_TEAM_SEASON_STATS_ACTIVATION_RESULT %s", json.dumps(result, default=str)
            )
            _msf_team_season_stats_logger.warning("MSF_TEAM_SEASON_STATS_ACTIVATION_DONE")
