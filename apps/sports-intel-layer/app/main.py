...
if os.environ.get("RAILWAY_ENVIRONMENT_NAME", "dev") == "dev":
    ...
    if os.environ.get("RUN_GNEWS_VALIDATION") == "1":
        import json
        import logging

        from app.master_refresh.production_clients import build_gnews_validation_client

        _gnews_validation_logger = logging.getLogger("sports-intel-layer.diagnostics.gnews_validation")

        @app.on_event("startup")
        async def _run_gnews_validation_once() -> None:
            """TEMPORARY, one-shot diagnostic hook for the 2026-09-03 MANSA
            News Provider Validation -- GNews (see
            `app.diagnostics.gnews_validation`'s own module docstring for
            the full call plan). Dev-only mount, gated behind
            `RUN_GNEWS_VALIDATION == "1"` so it never fires on an ordinary
            dev deploy. Same startup-hook + `logger.warning` retrieval
            pattern as both prior NFL provider bake-offs (this workspace's
            own egress policy cannot reach either the vendor domain or
            this service's own public Railway domain, so results go
            through Railway's own deploy logs instead of an HTTP
            response). The News provider credential is never referenced
            by name anywhere in this module; it is read exactly once, by
            `build_gnews_validation_client()` in
            `app.master_refresh.production_clients`."""
            client_and_key = build_gnews_validation_client()
            if client_and_key is None:
                _gnews_validation_logger.warning("GNEWS_VALIDATION_SKIPPED_NO_CREDENTIAL")
                return

            from app.diagnostics.gnews_validation import run_gnews_validation

            client, api_key = client_and_key
            try:
                result = await run_gnews_validation(client, api_key)
            finally:
                await client.aclose()

            _gnews_validation_logger.warning("GNEWS_VALIDATION_START")
            for call in result.get("calls", []):
                _gnews_validation_logger.warning("GNEWS_VALIDATION_CALL %s", json.dumps(call, default=str))
            if "note" in result:
                _gnews_validation_logger.warning("GNEWS_VALIDATION_NOTE %s", json.dumps({"note": result["note"]}))
            _gnews_validation_logger.warning("GNEWS_VALIDATION_DONE")
