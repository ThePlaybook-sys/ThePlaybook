...
if os.environ.get("RAILWAY_ENVIRONMENT_NAME", "dev") == "dev":
    ...
    if os.environ.get("RUN_MSF_PLAYER_GAMELOGS_DIAGNOSTIC") == "1":
        import json
        import logging

        _msf_gamelogs_diag_logger = logging.getLogger(
            "sports-intel-layer.diagnostics.msf_player_gamelogs_diagnostic"
        )

        @app.on_event("startup")
        async def _run_msf_player_gamelogs_diagnostic_once() -> None:
            """TEMPORARY, one-shot diagnostic hook for MANSA Phase 8.4B's
            HQ-authorized player-gamelogs live diagnostic (2026-09-08,
            see `app.diagnostics.msf_player_gamelogs_diagnostic`'s own
            module docstring). Exactly ONE real MySportsFeeds call,
            guarded by the same `activation_run_markers` mechanism every
            prior Phase 8.3 pass used (a new dedicated run_key, not a
            second idempotency mechanism). Diagnostic only -- captures
            and logs the raw response in size-bounded chunks (never one
            line, per HQ's explicit "no single 80KB+ log line"
            instruction this pass), persists nothing to `player_stats`.

            Exception-safe end to end, matching Phase 8.3C's own fix: an
            unexpected failure anywhere in this hook is logged, never
            re-raised, so a diagnostic failure of any kind can never
            crash startup or fail the healthcheck. Dev-only mount, gated
            behind `RUN_MSF_PLAYER_GAMELOGS_DIAGNOSTIC == "1"`."""
            from app.diagnostics.msf_player_gamelogs_diagnostic import (
                chunk_for_logging,
                run_msf_player_gamelogs_diagnostic,
            )

            _msf_gamelogs_diag_logger.warning("MSF_PLAYER_GAMELOGS_DIAGNOSTIC_START")
            try:
                result = await run_msf_player_gamelogs_diagnostic()
            except Exception as exc:  # last-resort guard -- a diagnostic must never crash startup
                _msf_gamelogs_diag_logger.error(
                    "MSF_PLAYER_GAMELOGS_DIAGNOSTIC_UNEXPECTED_FAILURE %s", exc, exc_info=True
                )
                _msf_gamelogs_diag_logger.warning("MSF_PLAYER_GAMELOGS_DIAGNOSTIC_DONE")
                return

            # Log the response body separately, in size-bounded chunks --
            # never as part of one combined JSON line, which is exactly
            # what truncated Phase 8.3C's capture at Railway's 80KB
            # single-log-line limit.
            body_text = result.pop("response_body_text", None)
            _msf_gamelogs_diag_logger.warning(
                "MSF_PLAYER_GAMELOGS_DIAGNOSTIC_RESULT %s", json.dumps(result, default=str)
            )
            if body_text:
                ...
                for index, chunk in enumerate(chunks, start=1):
                    _msf_gamelogs_diag_logger.warning(
                        "MSF_PLAYER_GAMELOGS_RAW_CHUNK %d/%d %s", index, total, chunk
                    )
            _msf_gamelogs_diag_logger.warning("MSF_PLAYER_GAMELOGS_DIAGNOSTIC_DONE")
