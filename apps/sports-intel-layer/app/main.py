...
if os.environ.get("RAILWAY_ENVIRONMENT_NAME", "dev") == "dev":
    ...
    if os.environ.get("RUN_MSF_PLAYER_STATS_DIAGNOSTIC") == "1":
        ...
        async def _run_msf_player_stats_diagnostic_once() -> None:
            """TEMPORARY, one-shot diagnostic hook for MANSA Phase 8.3C's
            HQ-authorized player-stats diagnostic retry (2026-09-08, see
            `app.diagnostics.msf_player_stats_diagnostic`'s own module
            docstring). Exactly ONE real MySportsFeeds call, guarded by
            the same `activation_run_markers` mechanism Phase 8.3A/8.3B
            both used (a new dedicated run_key, not a second idempotency
            mechanism). Diagnostic only -- captures and logs the raw
            response, persists nothing to `team_stats`/`player_stats`.

            Phase 8.3B's own uncaught timeout crashed application startup
            -- `run_msf_player_stats_diagnostic()` itself is now
            exception-safe end to end, and this hook wraps the call in
            one more layer of defense: an unexpected failure here is
            logged, never re-raised, so a diagnostic failure of any kind
            can never take the service down or fail its healthcheck.
            ...
            try:
                ...
                _msf_player_stats_diag_logger.error(
                    "MSF_PLAYER_STATS_DIAGNOSTIC_UNEXPECTED_FAILURE %s", exc, exc_info=True
                )
            else:
