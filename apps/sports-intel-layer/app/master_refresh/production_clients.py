...
def build_msf_players_diagnostic_client() -> tuple[httpx.AsyncClient, str] | None:
    """Phase 8.2 Players Identity Diagnostic #2 (2026-09-08, diagnostic
    only -- see `app.diagnostics.msf_players_diagnostic`): returns
    `(client, api_key)` bound to `MYSPORTSFEEDS_API_KEY`, or `None` if
    that credential isn't configured. Timeout raised to 120.0s (from
    pass #1's 30.0s, since reverted) -- HQ's own diagnosis of pass #1's
    `ReadTimeout` is that `/nfl/players.json` is a whole-league,
    un-scoped payload needing more time than the 30s bake-off default
    ever accounted for. `MYSPORTSFEEDS_API_KEY` is never read anywhere
    outside this function, matching the isolation convention every other
