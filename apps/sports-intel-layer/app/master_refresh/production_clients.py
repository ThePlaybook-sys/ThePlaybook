...
#: MySportsFeeds v2.1 base URL, diagnostic use only (Phase 8.3C Player
#: Stats Diagnostic Retry, 2026-09-08 -- see `app.diagnostics.
#: msf_player_stats_diagnostic`). CONFIRMED from the official
...
def build_msf_player_stats_diagnostic_client() -> tuple[httpx.AsyncClient, str] | None:
    """Phase 8.3C Player Stats Diagnostic Retry (2026-09-08, diagnostic
    only -- see `app.diagnostics.msf_player_stats_diagnostic`): returns
    `(client, api_key)` bound to `MYSPORTSFEEDS_API_KEY`, or `None` if
    that credential isn't configured. Timeout raised to 120.0s (from
    Phase 8.3B's 30.0s, which timed out with no response) -- matches the
    same 120.0s Phase 8.2's Players Identity Diagnostic #2 already proved
    necessary for a different MySportsFeeds v2.1 feed after an identical
    30.0s-timeout failure. `MYSPORTSFEEDS_API_KEY` is never read anywhere
