...
#: MySportsFeeds v2.1 base URL, diagnostic use only (Phase 8.4B Player
#: Gamelogs Live Diagnostic, 2026-09-08 -- see `app.diagnostics.
#: msf_player_gamelogs_diagnostic`). CONFIRMED from the official
#: `mysportsfeeds-node` npm package source (`API_v2_1.js`), identical to
#: every prior MSF diagnostic's own value (all since reverted).
...
def build_msf_player_gamelogs_diagnostic_client() -> tuple[httpx.AsyncClient, str] | None:
    """Phase 8.4B Player Gamelogs Live Diagnostic (2026-09-08, diagnostic
    only -- see `app.diagnostics.msf_player_gamelogs_diagnostic`):
    returns `(client, api_key)` bound to `MYSPORTSFEEDS_API_KEY`, or
    `None` if that credential isn't configured. Timeout 120.0s -- the
    longer timeout Phase 8.3C already proved necessary for a real
    MySportsFeeds v2.1 request after Phase 8.3B's 30.0s timeout failure,
    applied proactively here rather than rediscovered. `MYSPORTSFEEDS_
    API_KEY` is never read anywhere outside this function, matching the
    isolation convention every other provider credential in this module
    already follows. The caller owns closing the returned client."""
