...
# --------------------------------------------------------------- the clock
# These kickoffs used to be pinned to absolute wall-clock instants
# (2026-09-14). That worked on the day it was written and then rotted: the
# five tests below that go through the REAL HTTP endpoint cannot inject a
# fake clock (see `_run_one_cycle`'s docstring -- the endpoint deliberately
# has none), so they read `datetime.now(timezone.utc)` for real. Once that
# real date passed the pinned kickoff, `classify_window` correctly returned
# STOPPED for every fixture game and `games_due` went to 0 -- the tests were
# failing on their own staleness, not on any regression in the mechanism
# they exist to prove.
#
# The anchor is therefore RELATIVE and evaluated per test, at test time.
# `t0` is the test's own "now"; `kickoff` sits 50 minutes ahead of it, which
# is what puts the game in RAMP_60M (interval 900s) -- the window every
# assertion below is written against. `classify_window`'s RAMP_60M band runs
# from >15min to <=60min before kickoff, so anchoring at 50min leaves 35
# minutes of slack before the suite could drift into RAMP_15M and 10 before
# RAMP_2H: the file can no longer expire, and no assertion changed.
@pytest.fixture
def t0() -> datetime:
...
@pytest.fixture
def kickoff(t0: datetime) -> datetime:
    return t0 + timedelta(minutes=50)


@pytest.fixture
def game_row(kickoff: datetime) -> dict:
    return _game_row(game_id=DB_GAME_KC_BAL, home="KC", away="BAL", scheduled_start=kickoff.isoformat())
...
@respx.mock
def test_first_ever_invocation_treats_the_game_as_due(monkeypatch, t0, game_row):
    _set_env(monkeypatch)
    _mock_games_at([game_row])
    _mock_team_provider_ids()
    _mock_game_provider_ids(existing={GAME_CHIEFS_RAVENS: DB_GAME_KC_BAL})
    _mock_odds_snapshots_stateful({"now": t0})
...
@respx.mock
def test_immediate_second_stateless_invocation_is_not_due(monkeypatch, t0, game_row):
...
@pytest.mark.asyncio
@respx.mock
async def test_due_state_returns_after_the_correct_elapsed_window(monkeypatch, t0, game_row):
    ...
    first = await _run_one_cycle(now=t0)
    clock["now"] = t0 + timedelta(minutes=16)
    third = await _run_one_cycle(now=t0 + timedelta(minutes=16))
...
@respx.mock
def test_failed_provider_fetch_does_not_advance_cadence_state(monkeypatch, t0, game_row):
...
@respx.mock
def test_unresolved_event_does_not_advance_cadence_state(monkeypatch, t0, game_row):
...
@pytest.mark.asyncio
@respx.mock
async def test_post_kickoff_game_never_becomes_due_regardless_of_capture_history(monkeypatch, t0):
    ...
    already_kicked_off = _game_row(
        game_id=DB_GAME_DAL_PHI, home="DAL", away="PHI", scheduled_start=(t0 - timedelta(hours=1)).isoformat()
    )
    ...
    result = await _run_one_cycle(now=t0)
...
@respx.mock
def test_all_games_recently_captured_means_zero_provider_calls_end_to_end(monkeypatch, t0, kickoff):
    ...
    _set_env(monkeypatch)
    games = [
        _game_row(game_id=DB_GAME_KC_BAL, home="KC", away="BAL", scheduled_start=kickoff.isoformat()),
        _game_row(
            game_id=DB_GAME_DAL_PHI, home="DAL", away="PHI", scheduled_start=(t0 + timedelta(minutes=50)).isoformat()
        ),
        _game_row(
            game_id=DB_GAME_SF_BUF, home="SF", away="BUF", scheduled_start=(t0 + timedelta(minutes=50)).isoformat()
        ),
    ]
