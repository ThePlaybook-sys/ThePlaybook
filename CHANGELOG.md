...
## v5.0.2 (Volume 5) — 2026-08-28 — PATCH

**Volumes affected:** Volume 5 (Frontend & UX Architecture)

**Reason:** Phase 6 Milestone 2 shipped six real, thin read-only API routes. Volume 5 v5.0's §5 described the intended component contracts but named no literal route/response shape — HQ's own M2 instructions required documenting the exact final route/response names once built.

**Decision:** New §11 added: the six routes (`/v1/recommendations/today`, `/v1/recommendations`, `/v1/recommendations/{displayId}`, `/v1/recommendations/{displayId}/reconstruction`, `/v1/track-record`, `/v1/user/subscription`), their tier-gating behavior, data sources, and the exact neutral-ordering/freshness/Track-Record-scope rules the implementation enforces. Also documents a real gap found in `recommendation_products_tier_gated_select` (the RLS policy only handles `min_required_tier in ('free','pro','elite')`; a hypothetical `'syndicate'` value would be denied to everyone) — deliberately mirrored, not fixed, in the new `app.entitlement.tier_permits` (api-gateway), since fixing it in the API would make the API more permissive than the database's own real policy.

**Alternatives considered:** Documenting the API contract only in code comments, not the Blueprint. Rejected — Volume 5 is this project's authoritative frontend/API-contract reference; a route a frontend engineer needs to know about belongs there, not only in `apps/api-gateway`'s source.

**Expected impact:** Documentation only. No architectural change beyond what Milestone 2's own code entry (below) already made.

---

## v4.2 (apps/api-gateway, apps/ai-orchestrator) — 2026-08-28 — Phase 6 Milestone 2: Thin Read-Only Product API Exposure

**Volumes affected:** Volume 5 (v5.0.2 entry above) — implementation entry, not itself a Blueprint decision.

**Reason:** Phase 6's entry gate authorized Milestone 2 after Milestone 1 (Design System) closed: expose already-existing Phase 1-5 truth to the frontend without creating new product intelligence, per HQ's explicit scope and rules.

**Decision:** Built on `apps/api-gateway`: `app.supabase_client` (shared client helper), `app.entitlement` (tier-gating mirrored literally from `recommendation_products_tier_gated_select`, including its `'syndicate'` gap — see the Volume 5 v5.0.2 entry), `app.recommendations` (today/feed/detail/reconstruction-proxy routes), `app.track_record` (product-level W/L/P/V + sample size), `app.subscription` (own tier/status). Built on `apps/ai-orchestrator`: `GET /v1/internal/reconstruction/{id}`, a thin `dataclasses.asdict` wrapper around Milestone 5.3's already-tested `reconstruct_recommendation_product` — reused verbatim, per HQ's explicit instruction not to rebuild historical reasoning in the API layer. `apps/api-gateway/app/internal_client.py` gained `call_ai_orchestrator`, mirroring `call_sports_intel_layer`'s exact existing pattern; a new `AI_ORCHESTRATOR_URL` env var set on `api-gateway` (dev), `http://ai-orchestrator.railway.internal:8080`, per the PORT=8080 convention this project's Pre-Phase-6 gate already confirmed live for every internal service.

**Pre-implementation inspection findings (recorded per HQ's explicit mandatory-inspection requirement):** every Phase 5 table these routes touch besides `recommendation_products`/`games`/`subscriptions`/`user_profiles` has RLS enabled with **zero select policy** — a caller's own JWT could never read them via PostgREST regardless of this milestone's existence, so every new route reads via the service-role key (mirroring `main.py`'s own two existing routes) and enforces tier-gating in Python instead. `games.scheduled_start` (not null, populated at Schedule-ingestion time) is the authoritative neutral ordering key for game-scoped cards; `recommendation_activation_snapshots.activated_at` (also not null) is the fallback for slate-scoped ones — resolving the ordering question Pass 3 of Phase 6 planning left open. The Track Record stop-condition was checked and cleared for every currently-active `recommendation_type`: `single`/`player_prop` pass the leg outcome through verbatim, `multiple_singles` gets a real, already-wired `MIXED_SETTLED` product-level rollup (`app.features.grading.rollup_product_outcome`, called from production code, not a stub), and `no_bet`/`bankroll_preservation` are confirmed to only ever receive `NOT_APPLICABLE` — no gap required narrowing the endpoint below what HQ approved.

**Full test/regression evidence:** 12 new tests in `apps/api-gateway/tests/test_recommendations.py` (auth, No Bet/Bankroll Preservation/withdrawn serialization, tier-gating both directions, neutral-ordering proof using a lower-EV/lower-confidence-but-earlier-kickoff card, reconstruction 404/tier-hidden/success/transport-failure), 4 in `test_recommendation_detail.py` (Layers 1-4, tier-gating), 7 in `test_track_record.py` (zero-sample, `multiple_singles` counted once not per-leg, `NOT_APPLICABLE`/`PENDING_MISSING_DATA` excluded, correction-chain de-duplication, tier-gating, soft-delete exclusion), 3 in `test_subscription.py`, 13 in `test_entitlement.py` (parametrized tier matrix including the discovered `'syndicate'` gap), 3 in `apps/ai-orchestrator/tests/test_reconstruction_endpoint.py`. Full regression: api-gateway 66/66, ai-orchestrator 742/742, sports-intel-layer 618/618 (untouched, confirmed), workers 38/38 (untouched, confirmed) — 1,464/1,464 total, zero failures.

**Alternatives considered:** Duplicating `reconstruct_recommendation_product`'s logic directly in api-gateway (querying Supabase itself) instead of proxying to ai-orchestrator. Rejected per HQ's explicit instruction — this milestone reuses existing capability, it doesn't re-derive history. Hardcoding `min_required_tier`'s `'syndicate'` handling to be "correct" (i.e., permissive) in the new Python mirror. Rejected — would make the API more permissive than the database's own real, currently-live policy; the gap is dormant (no row uses that value) and is flagged, not silently improved past what production actually enforces today.

**Expected impact:** No new betting intelligence, ranking, EV/probability computation, or business logic — confirmed by design and by the test suite's own assertions (e.g. `test_neutral_ordering_uses_game_start_time_not_confidence_or_ev`, the Category-C-key-absence assertions in `test_track_record.py`). `/today`, `/recommendations`, `/track-record`, `/account` screens (Milestone 3+) now have a real, tested API to build against.

---

## v5.0.3 (Volume 5) — 2026-08-31 — PATCH

**Volumes affected:** Volume 5 (Frontend & UX Architecture)

**Reason:** Building Milestone 3's card components against §5's `RecommendationCardProps` surfaced that Milestone 2's shipped API (§11) has no route or field exposing grade/outcome data at all — `recommendation_products.status` is DB-constrained to `'active'|'withdrawn'` only, and grade state lives entirely in `recommendation_product_grade_events`, a table none of the three product routes ever read. STOPped per HQ's explicit M3 instruction ("if the API contract lacks something required by the approved UI: STOP") rather than guessing at a shape or building around it. HQ classified this Milestone 2.1 and approved Option 1 (extend the existing read contracts).

**Decision:** §11's three product routes (`/today`, `/recommendations`, `/{displayId}`) now each carry an additive `grade` field — `null` when ungraded, otherwise `{outcome, gradedAt, isCorrection, correctedAt}`. §5's `RecommendationCardProps` corrected to match: `status` stays `'active'|'withdrawn'` (never `'graded'`), and grade state moves into the new nested `grade` object rather than the flat `outcome`/`isCorrection` fields the original v5.0 draft had incorrectly attached directly to the card props. Lifecycle status and grade state are documented as independent dimensions per HQ's explicit domain rule.

**Alternatives considered:** Descoping graded/corrected card states out of Milestone 3 until a later milestone. Rejected by HQ — grading is core to the approved card variant set, not deferrable. A brand-new dedicated `/grade` endpoint instead of extending the existing routes. Not needed — the existing routes already batch-fetch per-product context (games, legs, explanations) by the same `product_ids` list: extending that batch with one more table is a small additive read, not new endpoint surface, and keeps the frontend to one round trip per page.

**Expected impact:** Documentation only — matches the Milestone 2.1 code entry below. No change to existing M2 response fields (backward compatible); Milestone 3 can now build every approved card variant against a real contract.

---

## v4.3 (apps/api-gateway) — 2026-08-31 — Phase 6 Milestone 2.1: Additive Product Grade Contract Correction

**Volumes affected:** Volume 5 (v5.0.3 entry above) — implementation entry, not itself a Blueprint decision.

**Reason:** Milestone 3 (Core Recommendation Experience) STOPped before writing any card/page code because Milestone 2's read routes had no way to expose grade/outcome data — confirmed by direct inspection (`recommendation_products.status` CHECK constraint is `'active'|'withdrawn'` only; grade lives in `recommendation_product_grade_events`, unread by `app.recommendations`). HQ authorized a small additive correction rather than a new milestone-scale build.

**Decision:** `apps/api-gateway/app/recommendations.py` gained `_read_grade_events_by_product_ids` (batched read, same `product_ids`-keyed pattern as every other context fetch in this module) and `_current_grade` (pure resolution function). `_serialize_product` gained an optional `grade` parameter, wired into `_visible_products_with_context` (list/today routes) and `get_recommendation_detail`, always scoped to already tier-authorized products only — never fetched for a product the request isn't entitled to see. **Current-outcome resolution reuses the exact rule `app.track_record.get_track_record` already established in this codebase** (Milestone 2): the append-only correction chain always appends a strictly-later `computed_at` row, so the latest row is authoritative — no second merge/reconciliation algorithm was invented. `gradedAt` is the original grade event's `computed_at`; `correctedAt` is set only when the current row is itself a correction, giving the frontend a real date for the "result corrected" sub-label without ever reconstructing the chain itself. Per-leg grade exposure was inspected and not added — no field in Volume 5's approved card or four-layer detail contracts reads a per-leg outcome, and `MIXED_SETTLED` is already the authoritative product-level value.

**Full test/regression evidence:** 14 new tests in `apps/api-gateway/tests/test_recommendation_grade_contract.py` (null-when-ungraded, WIN/LOSS/PUSH/VOID_NO_ACTION/MIXED_SETTLED/NOT_APPLICABLE outcome passthrough, correction-chain current-outcome + original-`gradedAt` preservation, lifecycle status never mutated into `'graded'`, withdrawn-status/grade independence, tier-gated 404 unchanged, list route + today route inclusion and field backward-compatibility). Existing `test_recommendations.py`/`test_recommendation_detail.py` mocks extended with an (empty-by-default) `recommendation_product_grade_events` route to stay green under the new read. Full api-gateway suite: 79/79 passing (65 pre-existing + 14 new). Relevant ai-orchestrator grading regression re-run for sanity (no ai-orchestrator code touched): `tests/features/test_grading.py`, `tests/orchestration/test_postgame_grading.py`, `tests/persistence/test_postgame_grading_persistence.py` — 32/32 passing, unaffected.
...
## v5.1 (Volume 5) — 2026-09-01 — MINOR

**Volumes affected:** Volume 5 (Frontend & UX Architecture)

**Reason:** Phase 6 Milestone 6 (Auth + Onboarding + Account) implemented the real sign-in/session architecture and the `/onboarding`/`/account` routes the IA table (§3) already named but nothing had built yet. M3's placeholder `pb_session_token` cookie -- explicitly documented as a stand-in "M6 doesn't exist to set yet" -- needed replacing with a real, working session, and §2 needed to describe what actually got built.

**Decision:** §2 gains the real `@supabase/ssr` session architecture (server/browser client split, `middleware.ts`'s `getUser()`-based revalidation, `api.ts`'s session-token-based gateway calls). §3's IA table gains `/sign-in` and `/account/how-it-works`, and corrects `/account`/`/onboarding`'s purpose text to what M6 actually built. §11 gains the two additive routes M6 is the first frontend milestone to consume (`GET`/`PATCH /v1/user/profile`, both unchanged since Phase 2 Milestone 4). A new implementation note records the actual component contracts (`AuthForm`, `OnboardingForm`, `CoreConcepts`, `AccountSummary`, `AppNav`) and the Telegram future-proofing analysis: no schema change is needed now, since `auth.users`/`user_profiles`' existing 1:1 identity model already lets a future external-identity table FK onto the same canonical user id.

**Alternatives considered:** A combined backend endpoint bundling profile+subscription+onboarding-status into one response. Rejected -- both existing routes are already correct and independently useful (subscription display doesn't need onboarding status, and vice versa); composing two already-tested reads client-side, the same choice M4 made for Time Machine, adds no new contract to maintain. `useFormState`/`useFormStatus` for the onboarding form (originally attempted). Rejected once confirmed these hooks aren't exported by this project's plain `react-dom@18.3.1` npm package outside Next's own build pipeline -- would have worked in `next build` but broken every test importing the component; a plain client-submit-handler (matching `AuthForm`'s already-established pattern) is functionally identical and fully testable.

**Expected impact:** Documentation only, matching the Milestone 6 code entry (below).

---

## v4.7 (apps/frontend) — 2026-09-01 — Phase 6 Milestone 6: Auth + Onboarding + Account

**Volumes affected:** Volume 5 (v5.1 entry above) — implementation entry, not itself a Blueprint decision.

**Reason:** HQ authorized Milestone 6 after M5's close-out, scoped to `/onboarding`, `/account`, and the minimum real user-facing authentication needed to reach every existing protected Phase 6 route safely and coherently -- explicitly not Telegram, not a second auth system, not weakened backend protections, and not a marketing site.

**Decision:** Added `@supabase/ssr`/`@supabase/supabase-js`. `app/lib/supabase/server.ts` (SSR client for Server Components/Actions/Route Handlers) and `client.ts` (the one browser client, used only for `auth.*` -- this codebase still never queries Supabase tables directly from the browser). `middleware.ts` refreshes/revalidates the session on every navigable request via `getUser()`. `app/lib/api.ts`'s `callGateway` now sources its bearer token from the real SSR session instead of the placeholder cookie; gained `getUserProfile`/`updateOnboarding`/`getSubscription`. `app/lib/auth.ts` gained the pure, independently-tested `resolveRootDestination` (signed-out → `/sign-in`; signed-in + `onboarding_completed_at` null → `/onboarding`; else → `/today`) plus `getCurrentUser`/`requireUser`/`redirectToRootDestination`. `app/page.tsx` (previously a static "The Playbook" stub) now performs real entry routing only -- no marketing content, per HQ's explicit boundary. Built `/sign-in` (`AuthForm`, two-tab email/password via Supabase Auth directly), `/auth/callback` (code-exchange Route Handler for email confirmation/magic links), `/auth/sign-out` (POST-only sign-out), `/onboarding` (`CoreConcepts` first-use education + `OnboardingForm`, one field: `jurisdiction_state`), `/account` (`AccountSummary` -- identity, jurisdiction, subscription tier/status/billing period/renewal, link to How The Playbook Works, sign-out), and `/account/how-it-works` (reopenable education: the four core concepts, the recommendation process, links to Time Machine and Track Record). Added `components/nav/AppNav` (persistent nav across all five authenticated destinations, wired into `/today`, `/recommendations`, `/recommendations/[displayId]`, `/history`, `/history/[displayId]`, `/track-record`, `/account`, `/account/how-it-works`) -- without it those routes were only reachable by typing a URL directly. Set `NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY` on Railway dev/frontend as references to the already-set `SUPABASE_URL`/`SUPABASE_ANON_KEY` (`skipDeploys: true` per CLAUDE.md's Railway-mutation rule) -- no secret value ever read or copied by hand.

**Auth architecture, precisely:** email/password only (OAuth remains deferred, unauthorized). No second authentication system, no bypass, no DEV-only shortcut, no service-role exposure to the browser (confirmed `SUPABASE_SERVICE_ROLE_KEY` is not among frontend's Railway variables). Backend protections unchanged -- api-gateway's `get_current_user` remains the sole authority on token validity; the frontend only ever forwards whatever the SSR session currently holds.

**Email-confirmation verification, disclosed rather than assumed:** HQ's authorization required verifying DEV's actual email-confirmation configuration before relying on it. Two paths were attempted -- a direct `GET {SUPABASE_URL}/auth/v1/settings` (GoTrue's own public config endpoint, no auth required) and checking for a Supabase Management API token in this session's environment -- both blocked or unavailable (this sandbox's egress policy 403-denies direct HTTPS to `*.supabase.co`, the same class of restriction that has blocked visual validation since M3; no `SUPABASE_ACCESS_TOKEN` exists in this session). No Supabase MCP tool exposes GoTrue auth settings either. Resolved without guessing: `AuthForm` branches on the real `signInWithPassword`/`signUp` response rather than assuming either configuration -- a returned session proceeds immediately, no session shows an honest "check your email" state. Correct under either setting; this is not a STOP-condition trigger because the implementation never assumes one.

**Telegram future-proofing:** no code, table, or endpoint for Telegram exists or was added. The existing identity model (Supabase-managed `auth.users` as the one canonical user id, `user_profiles.id` FK'd 1:1 to it) already accommodates a future external-identity table without redesign -- confirmed by inspection, not by adding speculative schema now.

**Full test/regression evidence:** 91/91 frontend tests passing (58 pre-existing + 33 new: 5 `resolveRootDestination` pure-function cases covering all three routing outcomes plus a signed-out-always-wins edge case; 4 `getCurrentUser`/`requireUser` cases covering a valid session, signed-out, a stale/invalid session -- `getUser()` returning no user on a revalidation failure -- and the `/sign-in` redirect; 5 `AuthForm` cases covering sign-in success, real-error-message-on-failure, awaiting-confirmation, immediate-session sign-up, and mobile-safe 16px/44px structure; 4 `OnboardingForm` cases covering submission, real error display, native `required` validation, and mobile-safe structure; 5 `CoreConcepts` cases covering all four concepts, the exact confidence-is-not-win-probability wording, the No-Bet-is-legitimate wording, no "MANSA" text, and the `compact` prop; 5 `AccountSummary` cases covering identity/jurisdiction display, an active subscription, an honest no-subscription state, an honest read-failure state -- never silently rendered as "no subscription" -- and the sign-out form; 2 `getUserProfile`/`updateOnboarding`/`getSubscription` `api.ts` cases; 2 `AppNav` cases covering all five links and mobile-safe horizontal-scroll structure; plus the pre-existing `api.ts` suite updated to mock the new Supabase-session token source instead of the retired raw cookie). `tsc --noEmit` clean. `npm run build` clean, all 21 routes compiled, `middleware.ts` correctly built (56.2 kB).
